...
HOST="${1:-Vast002}"
LLAMA_MODEL="${LLAMA_MODEL:-NousResearch/Meta-Llama-3.1-8B-Instruct}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-llama-3.1}"
...
REMOTE_ENV=(
  "LLAMA_MODEL=$(printf '%q' "${LLAMA_MODEL}")"
...
REMOTE_ROOT="/workspace/llama-3.1-vllm"
REMOTE_VENV="${REMOTE_ROOT}/.venv"
REMOTE_START="/workspace/start_llama31_vllm.sh"
...
export LD_LIBRARY_PATH="/workspace/llama-3.1-vllm/.venv/lib/python3.12/site-packages/nvidia/cu13/lib:${LD_LIBRARY_PATH:-}"
...
cd /workspace/llama-3.1-vllm
. /workspace/llama-3.1-vllm/.venv/bin/activate

args=(
  vllm serve "${LLAMA_MODEL}"
...
stopsignal=TERM
stopwaitsecs=20
stdout_logfile=/workspace/llama-3.1-vllm/vllm.log
...
echo "Llama 3.1 vLLM did not become ready within timeout. Recent supervisor status:" >&2
